...
v2ray_config() {
	# clear
	while :; do
		# echo -e "请选择 "$yellow"V2Ray"$none" 传输协议 [${magenta}1-${#transport[2]}$none]"
...
echo
				echo -e "$yellow V2Ray 端口(随机) = $cyan$v2ray_port$none"
				break
				;;
			*)
				error
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		pushd /tmp
		git clone https://github.com/lhl77/v2ray -b "$_gitbranch" /etc/v2ray/233boy/v2ray --depth=1
		popd
...
while :; do
	echo
	echo "........... 白嫖佬不会搭节点专用-一键v2ray搭建脚本.........."
	echo
	echo "修改自：233blog脚本"
	echo
	echo "讨论群：https://t.me/xbzijian"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
